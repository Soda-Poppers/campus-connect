...
const DiscoveryPage = () => {
    ...
    filters: {
      ...
      course:
        courseFilter !== "all-courses"
          ? (courseFilter as Course | undefined)
          : undefined,
  ...
  const filteredUsers = users.filter((user) => {
    ...
    if (debouncedSearchQuery) {
      ...
      const matchesSearch =
        ...
        user.Modules.some((module) =>
          module.module.classId.toLowerCase().includes(query),
        );
    ...
    return activeFilters.some((filter) => {
      return user.Modules.some(
        (module) => module.module.classId === filter.modId,
      );
    });
  });
  ...
  const UserCard = ({ user }: { user: Users }) => {
    if (viewMode === "list") {
      return (
        <Card className="border-border/50 card-animate border p-5 transition-all duration-200 hover:shadow-lg">
          <div className="flex items-center space-x-4">
            <Avatar className="border-primary/20 h-14 w-14 border-2">
              {/* <AvatarImage src={user.image} /> */}
              <AvatarFallback className="from-primary/10 to-secondary/10 text-primary bg-gradient-to-br font-medium">
                {user.name
                  .split(" ")
                  .map((n) => n[0])
                  .join("")}
    ...
    return (
      <Card className="border-border/50 card-animate border p-5 transition-all duration-200 hover:shadow-lg">
        <div className="text-center">
          <Avatar className="border-primary/20 mx-auto mb-4 h-20 w-20 border-2">
            <AvatarFallback className="from-primary/10 to-secondary/10 text-primary bg-gradient-to-br text-lg font-medium">
              {user.name
                .split(" ")
                .map((n) => n[0])
                .join("")}
          ...
          <h3 className="text-primary mb-1 text-lg font-medium">{user.name}</h3>
          <p className="text-muted-foreground mb-3 text-sm">
            Enrolled in {user.enrollmentYear}
          </p>
          <Badge variant="default" className="mb-3 px-3 py-1 text-xs">
            {user.course
              .replace(/_/g, " ")
              .toLowerCase()
              .replace(/\b\w/g, (char) => char.toUpperCase())}
          ...
          <div className="space-y-2">
            ...
            <Button size="sm" className="bg-primary mobile-button w-full">
              <MessageCircle className="mr-2 h-4 w-4" />
              Connect
            </Button>
          </div>
  ...
  return (
    <div className="safe-area-top touch-scroll h-full overflow-y-auto">
      <div className="space-y-6 p-6">
        {/* Header */}
        <div className="flex items-center justify-between">
          <h1 className="text-primary text-2xl font-bold">Discover Students</h1>
        ...
        <div className="relative">
          ...
          <Input
            ...
            className="mobile-form-input h-14 pl-12"
          />
          <Button
            variant="ghost"
            size="sm"
            className="mobile-button absolute top-1/2 right-2 -translate-y-1/2 transform"
            onClick={() => setShowFilters(!showFilters)}
          >
            <Filter
              className={`h-5 w-5 ${showFilters || activeFilters.length > 0 ? "text-primary" : ""}`}
            />
            <ChevronDown
        ...
        <div className="gap-4">
          <Select value={courseFilter} onValueChange={handleCourseChange}>
            <SelectTrigger className="mobile-form-input h-12">
              <SelectValue placeholder="Filter by degree" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all-courses">All Courses</SelectItem>
              {courseOptions.map((course) => (
                <SelectItem key={course.label} value={course.value}>
                  {course.label}
                </SelectItem>
              ))}
            </SelectContent>
        ...
        {/* Advanced Filters */}
        {showFilters && (
          <Card className="border-primary/20 card-animate space-y-6 p-6">
            <div className="flex items-center justify-between">
        ...
        <div className="text-muted-foreground">
          <span className="font-medium">{filteredUsers.length}</span> student
